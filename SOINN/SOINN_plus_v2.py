...
class SOINN_plus(object):
    ...
    def __init__(self, x1, x2, x3, pull_factor=100):
        self.t = 3
    ...
    def _edge_deletion(self, n1):
        ...
        for e in self.network.es:
            if e['lt'] > lt_mean:
                prob_deletion = random.uniform(0, 1)
                prob_survival = 1 - e['lt'] / max_lt
    ...
    def _nodes_deletion(self):
        ...
        for n in self.network.vs:
            if n.degree() == 0 and n['u'] < u_mean:
                prob_deletion = random.uniform(0, 1)
                # prob_survival is high when the utility of the node is high
